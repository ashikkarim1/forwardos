...
export async function GET(request: NextRequest) {
  const region = (request.nextUrl.searchParams.get('region') as Region) || 'USA'
  if (region !== 'USA' && region !== 'CANADA' && region !== 'UAE') {
    return NextResponse.json({ error: 'region must be USA, CANADA or UAE' }, { status: 400 })
  }
